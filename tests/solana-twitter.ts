...
describe("solana-twitter", () => {
	...
	it("can send a new tweet without a tweet", async () => {
		// Before sending the transaction to the blockchain.
		const tweet = anchor.web3.Keypair.generate();
	...
	it("can send a new tweet from another auther", async () => {
		...
		const tweet = anchor.web3.Keypair.generate();
		await program.rpc.sendTweet("veganism", "this is working", {
			accounts: {
				tweet: tweet.publicKey,
		...
		assert.equal(tweetAccount.topic, "veganism");
		assert.equal(tweetAccount.content, "this is working");
		assert.ok(tweetAccount.timestamp);
	});
	it("can send a new tweet with more then 50 char in topic", async () => {
		// Before sending the transaction to the blockchain.
		try {
			const tweet = anchor.web3.Keypair.generate();
			const xchar = "x".repeat(51);
			await program.rpc.sendTweet(xchar, "this is not working", {
				accounts: {
					tweet: tweet.publicKey,
					author: program.provider.wallet.publicKey,
					systemProgram: anchor.web3.SystemProgram.programId
	...
	it("cannot provide a content with more than 280 characters", async () => {
		try {
			const tweet = anchor.web3.Keypair.generate();
			const xchar = "x".repeat(281);
			await program.rpc.sendTweet("test", xchar, {
				accounts: {
					tweet: tweet.publicKey,
					author: program.provider.wallet.publicKey,
					systemProgram: anchor.web3.SystemProgram.programId
	...
	it("can filter tweets with author", async () => {
		const tweetAccounts = await program.account.tweet.all([
			{
				memcmp: {
					offset: 8,
		...
		assert.ok(
			tweetAccounts.every((tweet) => {
				return (
					tweet.account.author.toBase58() ===
					program.provider.wallet.publicKey.toBase58()
				);
			})
		);
	});
	it("can filter tweets with topics", async () => {
		const tweetAccounts = await program.account.tweet.all([
			{
				memcmp: {
					offset: 8 + 32 + 8 + 4,
